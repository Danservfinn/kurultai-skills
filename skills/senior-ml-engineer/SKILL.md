...
When senior-ml-engineer is invoked by Ralph Loop (parse-ralph-loop skill) for specialist review, it MUST provide a resumption signal at the end of its response.

**Detection**: If invoked as part of Parse Ralph Loop Phase 4 (CRITICAL REVIEW) for LLM_AGENT or SCORING_CALIBRATION issues.
...
**RALPH_LOOP_RESUME**: senior-ml-engineer review complete.

**Issue Category**: LLM_AGENT | SCORING_CALIBRATION
...
⚠️ **CRITICAL**: Always include the `RALPH_LOOP_RESUME` block when invoked by Ralph Loop.