...
def _search_reddit(
    ...
    """Search Reddit via OpenAI or ZAI (runs in thread).

    Uses OpenAI if available, falls back to ZAI if configured.
    ...
    reddit_provider = env.get_reddit_provider(config)

    if mock:
        raw_response = load_fixture("openai_sample.json")
        # Use OpenAI parser for mock data
        reddit_items = openai_reddit.parse_reddit_response(raw_response or {})
        return reddit_items, raw_response, reddit_error

    if reddit_provider == "zai":
        # Use ZAI for Reddit search
        try:
            raw_response = zai_reddit.search_reddit(
                config["ZAI_API_KEY"],
                selected_models["zai"],
                topic,
                from_date,
                to_date,
                depth=depth,
                base_url=config.get("ZAI_BASE_URL"),
            )
        except http.HTTPError as e:
            raw_response = {"error": str(e)}
            reddit_error = f"ZAI API error: {e}"
        except Exception as e:
            raw_response = {"error": str(e)}
            reddit_error = f"{type(e).__name__}: {e}"

        # Parse response
        reddit_items = zai_reddit.parse_reddit_response(raw_response or {})

        # Quick retry with simpler query if few results
        if len(reddit_items) < 5 and not reddit_error:
            core = zai_reddit._extract_core_subject(topic)
            if core.lower() != topic.lower():
                try:
                    retry_raw = zai_reddit.search_reddit(
                        config["ZAI_API_KEY"],
                        selected_models["zai"],
                        core,
                        from_date, to_date,
                    ...
                    retry_items = zai_reddit.parse_reddit_response(retry_raw)
        ...
        # Use OpenAI for Reddit search (default)
        try:
