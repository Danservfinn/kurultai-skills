...
Perform comprehensive multi-disciplinary reviews by dispatching specialized skills across UX, backend, frontend, architecture, data, and DevOps domains. This skill invokes expert skills for each domain in parallel, synthesizes findings into a consolidated report, and provides an automatic implementation pathway for approved improvements.
...
### Phase 2: Conduct Multi-Domain Analysis (Parallel Skill Dispatch)

Analyze the artifact by dispatching specialized skills in parallel for each relevant domain.
...
| Frontend | `senior-frontend` | Components, state management, responsiveness |
...
| Code Review | `code-reviewer` | Comprehensive code analysis |

**Dispatch Pattern:**

When multiple domains need analysis, use the `Skill` tool to invoke `dispatching-parallel-agents`:

1. Create a single message with multiple `Skill` tool calls (one for each domain skill)
2. Each skill receives focused prompt with artifact context
3. Skills execute in parallel without shared state
4. Collect and synthesize results after all complete

**Example parallel dispatch invocation:**
```python
# All in a single response - parallel execution
Skill("accessibility-auditor", "Review [artifact] for WCAG compliance and accessibility")
...
**Important:** Always invoke all parallel skills in a SINGLE response. This ensures they execute simultaneously rather than sequentially.
...
4. **Resolve conflicts** - When domains disagree:
   - **Present trade-off explicitly** using the conflict template:
     ```
     [DOMAIN A] suggests: [specific recommendation]
     [DOMAIN B] requires: [contradictory requirement]

     Trade-off: [brief explanation of the core conflict]
     ```
   - **Ask user to prioritize** with guiding questions:
     ```
     Which concern takes precedence for this artifact?
     - Prioritize [DOMAIN A] because: [context where A wins]
     - Prioritize [DOMAIN B] because: [context where B wins]
     - Hybrid approach: [possible compromise if one exists]
     ```
   - **Document the decision** in the report with a "Conflict Resolutions" section:
     ```markdown
     ## Conflict Resolutions

     | Conflict | Domains | Resolution | Rationale |
     |----------|---------|------------|-----------|
     | [brief description] | UX vs Security | [chosen path] | [user's reasoning] |
     ```
   - **Create decision record** when resolution affects architecture:
     - What was decided
     - Which domains were involved
     - What trade-offs were accepted
     - Any conditions for future re-evaluation
